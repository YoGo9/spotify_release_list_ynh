...
# Extract upstream sources into $install_dir, write .env, install deps, run
# the CRA build, then drop dev-only artifacts (node_modules, src) since the
# app is served as static files from $install_dir/build afterwards.
myynh_build_app() {
